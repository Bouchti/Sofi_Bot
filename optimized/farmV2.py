...
logging.basicConfig(level=logging.DEBUG)  # Set to INFO to include timing logs
...
def scrape_top_characters(limit=3000):
    ...
    end_time = time.time()
    logging.info(f"⏱️ Time taken for scrape_top_characters: {end_time - start_time:.2f} seconds")
    return characters
...
def click_lowest_or_first_card():
    ...
    end_time = time.time()
    logging.info(f"⏱️ Time taken for click_lowest_or_first_card: {end_time - start_time:.2f} seconds")
...
def focus_discord():
    ...
    for window in gw.getWindowsWithTitle("Discord"):
        ...
        time.sleep(1)
        end_time = time.time()
        logging.info(f"⏱️ Time taken for focus_discord: {end_time - start_time:.2f} seconds")
        return True
    end_time = time.time()
    logging.info(f"⏱️ Time taken for focus_discord: {end_time - start_time:.2f} seconds")
    return False
